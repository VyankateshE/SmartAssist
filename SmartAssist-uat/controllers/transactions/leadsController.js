...
// const PAGE5_FIELDS = {
//   productSpecialist: { x: 279, y: 975, size: 20},
// };

// const PAGE14_FIELDS = {
//   name: { x: 130, y: 1200, size: 20 },
//   contact: { x: 180, y: 540, size: 12 },
//   email: { x: 180, y: 515, size: 12 },
//   model: { x: 180, y: 405, size: 12 },
//   brand: { x: 360, y: 405, size: 12 },
//   vin: { x: 180, y: 380, size: 12 },
//   dealer: { x: 180, y: 355, size: 12 },
// };


// async function drawField(page, pdfDoc, text, field) {
//   if (!text || !field) return;

//   const font =
//     field.font === "HelveticaBold"
...
//       pdfDoc,
//       lead.lead_owner,
//       PAGE5_FIELDS.productSpecialist
//     );
...
//     await drawField(page14, pdfDoc, fullName, PAGE14_FIELDS.name);
//     await drawField(page14, pdfDoc, lead.mobile, PAGE14_FIELDS.contact);
//     await drawField(page14, pdfDoc, lead.email, PAGE14_FIELDS.email);
//     await drawField(page14, pdfDoc, lead.vehicle_name, PAGE14_FIELDS.model);
//     await drawField(page14, pdfDoc, lead.brand, PAGE14_FIELDS.brand);
//     await drawField(page14, pdfDoc, lead.VIN, PAGE14_FIELDS.vin);
//     await drawField(page14, pdfDoc, lead.dealer_name, PAGE14_FIELDS.dealer);
...
const TEMPLATE_PATH =
  "D:/Node_Folder/SmartAssist-uat/pdfFolder/250107_IND_Handover Guide_DEFENDER_V12_Justify_Approved_Interactive.html";

const BG_PAGE5 =
  "D:/Node_Folder/SmartAssist-uat/pdfFolder/img5.jpg";

const BG_PAGE14 =
  "D:/Node_Folder/SmartAssist-uat/pdfFolder/img14.jpg";
...
const downloadGuideWithProductSpecialist = async (req, res) => {
  try {
    ...
    html = html
      .replace("{{lead_owner}}", lead.lead_owner || "")
      .replace("{{full_name}}", fullName || "")
      .replace("{{mobile}}", lead.mobile || "")
      .replace("{{email}}", lead.email || "")
      .replace("{{vehicle}}", lead.vehicle_name || "")
      .replace("{{brand}}", lead.brand || "")
      .replace("{{vin}}", lead.VIN || "")
      .replace("{{dealer}}", lead.dealer_name || "")
      .replace("{{PAGE5_BG}}", toFileUrl(BG_PAGE5))
      .replace("{{PAGE14_BG}}", toFileUrl(BG_PAGE14));
    ...
    // IMPORTANT: provide base URL so images load
    await page.setContent(html, {
      waitUntil: "networkidle0",
      url: toFileUrl(TEMPLATE_PATH),
    });
